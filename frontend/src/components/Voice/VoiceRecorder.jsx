  ...
  return (
    <div>
      <Button
        ...
        style={{ marginRight: 8 }}
      >{recording ? '停止录音' : '开始录音'}</Button>
      {recording && (
        <Progress percent={Math.round((recordingTime / MAX_RECORD_SECONDS) * 100)} size="small" style={{ width: 120, display: 'inline-block', marginLeft: 12 }} />
      )}
      {/* 试听、取消等可按需扩展 */}
    </div>
  );
